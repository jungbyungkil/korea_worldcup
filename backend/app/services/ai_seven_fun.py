...
# --- Step 6: team fan lines ---


_TEAM_KO = {
    "mexico": "멕시코",
    "south_africa": "남아프리카 공화국",
    "playoff_d": "UEFA 플레이오프 D 승자",
}


async def step6_team_fan_lines(team_key: str) -> dict[str, Any]:
    k = (team_key or "").strip().lower().replace("-", "_")
    if k in ("southafrica",):
        k = "south_africa"
    label = _TEAM_KO.get(k)
    if not label:
        raise ValueError("team은 mexico, south_africa, playoff_d 중 하나여야 합니다.")
    system = (
        "Two one-liner fan perspectives before Korea vs opponent in 2026 WC group A. "
        "Return ONLY JSON: opponent_fan_line_ko (as if that team's fan, playful), "
        "korea_fan_line_ko (Korea fan clapback, playful), "
        "disclaimer_ko (fiction personas, for fun)."
    )
    user = f"상대: {label}. 한국 대표팀 관점과 상대 팬 관점 각 한 줄."
    p = await _chat_json(system, user, 0.88)
    return {
        "team": k,
        "opponent_fan_line_ko": str(p.get("opponent_fan_line_ko") or "")[:200],
        "korea_fan_line_ko": str(p.get("korea_fan_line_ko") or "")[:200],
        "disclaimer_ko": str(p.get("disclaimer_ko") or "")[:300],
    }
...
async def step7_home_daily(fixture_blob: dict[str, Any] | None) -> dict[str, Any]:
    system = (
        "Korean football app home card. Return ONLY JSON: "
        "headline_ko (max 36 chars), line_ko (2 sentences max, hype but sane), "
        "disclaimer_ko (fixtures from API; may change)."
    )
    if fixture_blob:
        user = f"다음은 한국대표 다음/최근 경기 힌트:\n{json.dumps(fixture_blob, ensure_ascii=False)}"
    else:
        user = "일정 데이터가 비었습니다. 2026 월드컵·한국 대표팀을 응원하는 짧은 홈 메시지를 써줘."
    p = await _chat_json(system, user, 0.86)
    return {
        "headline_ko": str(p.get("headline_ko") or "")[:80],
        "line_ko": str(p.get("line_ko") or "")[:400],
        "disclaimer_ko": str(p.get("disclaimer_ko") or "")[:300],
    }
