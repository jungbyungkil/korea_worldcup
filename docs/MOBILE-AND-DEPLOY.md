...
## 5. 멕시코·남아공 대표 페이지가 「대표팀을 찾지 못했습니다」일 때

- Render 등에 **`API_FOOTBALL_KEY`** 가 로컬과 동일하게 들어갔는지 확인합니다.
- 최신 코드에서는 `teams?search=` 결과를 **여러 페이지까지** 합쳐 국가대표를 찾습니다. 예전처럼 「Mexico」 검색 첫 페이지에 클럽만 있을 때 국가대표가 빠지던 문제를 줄입니다.
- 그래도 안 되면 API-Sports 대시보드에서 **멕시코 남자 국가대표**의 `team id`(대개 **16**)를 확인한 뒤, 백엔드 환경 변수에  
  **`API_FOOTBALL_TEAM_ID_MEXICO=16`** 을 넣고 재배포합니다. (`backend/.env.example` 참고)
